...
import uuid
import re
# [修改 1] 移除 cloudscraper，引入 curl_cffi
...
class NotionAIProvider(BaseProvider):
    def __init__(self):
        # [修改 2] 使用 curl_cffi 初始化 Session，伪装成 Chrome 120
        # 这解决了 Token 容易过期的问题，因为 TLS 指纹完全拟真
    ...
    def _warmup_session(self):
        try:
            ...
            # 访问主页以建立 TLS 会话和 Cookie
            response = self.session.get("https://www.notion.so/", headers=headers, timeout=30)
            if response.status_code != 200:
                logger.warning(f"预热请求返回状态码: {response.status_code}")
            logger.info("会话预热完成。")
    ...
    async def _create_thread(self, thread_type: str) -> str:
        ...
        try:
            ...
            # 使用 run_in_threadpool 调用同步的 session.post
            ...
            try:
                ...
                def sync_stream_iterator():
                    try:
                        ...
                        # [修改 3] 使用 curl_cffi 的 stream=True
                ...
                if incremental_fragments:
                    ...
                    logger.info(f"使用拼接所有增量片段的方式获得最终消息 (长度: {len(full_response)})。")
                elif final_message:
                    full_response = final_message
                    logger.info(f"流式片段为空，回退到使用 record-map/final 消息。")

                if full_response:
                    cleaned_response = self._clean_content(full_response)
                    # logger.info(f"清洗后的最终响应: {cleaned_response[:100]}...")
    ...
    def _prepare_headers(self) -> Dict[str, str]:
        ...
        # 简单的容错：如果用户只填了 token_v2 的值，自动补全 key
        ...
        return {
            ...
            # [修改 4] User-Agent 必须与 curl_cffi 的 impersonate 版本匹配 (Chrome 120)
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
        }

    # ... 剩下的辅助函数 (_normalize_block_id, _prepare_payload, _clean_content, _parse_ndjson_line_to_texts, get_models)
    # 保持原样即可，不需要修改。
    # 为了节省篇幅，我没有重复粘贴这些未修改的函数，请保留你原文件中的这些函数。
    ...
    def _parse_ndjson_line_to_texts(self, line: bytes) -> List[Tuple[str, str]]:
        # 这里完全保留你原有的解析逻辑，因为这是项目 A 稳定性的关键
        ...
        try:
            ...
            if data.get("type") == "markdown-chat":
                ...
                for operation in data.get("v", []):
                    ...
                    elif (op_type in ["x", "s", "a"]) and ("/value/" in path or path.endswith("/value")):
                        content = ""
                    ...
                    elif (op_type == "x" or op_type == "s" or op_type == "a") and "/value/" in path and isinstance(value, str):
                        content = value
                        if content:
                            results.append(('incremental', content))
